...
function getProductSignature(product) {
  const signature = [
    ...
    normalizeProductField(product?.category),
    Number(product?.price || 0),
    normalizeProductField(product?.image),
  ].join("|");

  return signature.replace(/\|/g, "") ? signature : `id:${product?.id}`;
}
...
function mergeDuplicateProducts(existing, incoming) {
  ...
  return {
    ...base,
    badge: base.badge || fallback.badge || "",
    desc: base.desc || fallback.desc || "",
    featured: Boolean(existing.featured || incoming.featured),
    sortOrder: Math.min(
      Number(existing.sortOrder ?? base.sortOrder ?? 0),
      Number(incoming.sortOrder ?? base.sortOrder ?? 0),
    ),
  };
}
...
function dedupeStoreProducts(products) {
  ...
  for (const product of products) {
    ...
    const existing = uniqueProducts.get(signature);

    uniqueProducts.set(
      signature,
      existing ? mergeDuplicateProducts(existing, product) : product,
    );
  }
